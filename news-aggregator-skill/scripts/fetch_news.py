...
def fetch_harness_engineering(limit=5, keyword=None):
    ...
    try:
        ...
        # Based on the site structure, articles are in blog posts
        for link in soup.find_all('a', href=True):
            href = link.get('href')
            
            # Filter for blog post URLs (handle both relative and absolute URLs)
            if '/blog/' not in href:
                continue
            ...
            # Try to find date from the parent element or nearby
            # Many articles have date info in the same container
            date_str = ""
            parent = link.find_parent()
            if parent:
                # Look for date patterns in the parent or siblings
                date_text = parent.get_text()
                # Common date patterns: "April 4, 2026", "April 4th, 2026"
                date_match = re.search(r'(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}(?:st|nd|rd|th)?,?\s+\d{4}', date_text)
                if date_match:
                    date_str = date_match.group(0)

            # If no date found, try to extract from title (many have "Daily AI Agent News Roundup — Month Day, Year")
            if not date_str:
        ...
        return filter_items(prioritized[:limit], keyword)
        
    except Exception as e:
        print(f"Harness Engineering fetch error: {e}", file=sys.stderr)
        return []
...
def main():
    ...
    sources_map = {
        ...
        'harness': fetch_harness_engineering,
        'harness_engineering': fetch_harness_engineering,
    }
